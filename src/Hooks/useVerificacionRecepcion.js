...
import { normalizarTextoRecepcion, traducirRolloRecepcion } from "../Utils/recepcion.js";

/** Verificación, resumen y confirmación de una recepción ya previsualizada. */
export function useVerificacionRecepcion({ sesion, archivo }) {
  ...
  const [rollosCrudos, setRollosCrudos] = useState([]); const [cargandoVerificacion, setCargandoVerificacion] = useState(false);
  const [confirmando, setConfirmando] = useState(false); const [errorConfirmacion, setErrorConfirmacion] = useState("");
  const [estadoFinal, setEstadoFinal] = useState(null); const [historialRecepciones, setHistorialRecepciones] = useState([]);
  async function verificarEnServidor() {
    setCargandoVerificacion(true); setErrorConfirmacion("");
    try { const respuesta = await api.post("/recepcion/verificar", { mapeo: archivo.mapeoColumnas, tolerancia_porcentaje: toleranciaPorcentaje }); setRollosCrudos(respuesta.rollos); }
  ...
  function actualizarTolerancia(valorTexto) { const valor = Number(valorTexto); if (Number.isFinite(valor) && valor >= 0) setToleranciaPorcentaje(valor); }
  async function confirmarRecepcion() {
    ...
    try {
      ...
      const creada = await api.post("/recepcion/confirmar", { tolerancia_porcentaje: toleranciaPorcentaje, proveedor_principal: proveedorPrincipal });
