...
export function redondearRecepcion(numero, decimales = 2) {
  const factor = 10 ** decimales;
  return Math.round(numero * factor) / factor;
}

export function normalizarTextoRecepcion(texto) {
  return (texto ?? "").toString().normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
}

export function traducirRolloRecepcion(rollo, indice, toleranciaPorcentaje) {
